import{seededRng,normalFrom,phaseFor,seedForEpisode,computeFill,computeCommission,stepReward}from'./lib/market-core.mjs';
const $=id=>document.getElementById(id),bctx=$('brain').getContext('2d'),STARTING_CAPITAL=20,HORIZON=480;
// ?control=1 arranca el worker en modo control (misma topologia, pesos aleatorios congelados,
// sin exploracion ni aprendizaje) para poder comparar el agente entrenado contra ese baseline
// sin tener que instrumentar un segundo worker en paralelo. Ver auditoria, punto 4/11.
const CONTROL_MODE=new URLSearchParams(location.search).get('control')==='1';
const STORAGE_PREFIX=CONTROL_MODE?'flybrain-v2-control':'flybrain-v2',HISTORY_KEY=STORAGE_PREFIX+'-history',CHECKPOINT_KEY=STORAGE_PREFIX+'-checkpoint',EPISODE_KEY=STORAGE_PREFIX+'-episode';
const ACTIONS=['BUY 10','BUY 25','BUY 50','HOLD','REDUCE','EXIT','ABSTAIN'],COMMISSION_BPS=8,SPREAD_BPS=4,SLIPPAGE_BPS=2,MIN_HOLD=5,COOLDOWN=3,MAX_TRADES=20,TRADE_RISK=.0025,EPISODE_LOSS=.01,persistedHistory=load(HISTORY_KEY,[]);
const state={running:true,speed:4,episode:load(EPISODE_KEY,0),step:0,price:100,cash:STARTING_CAPITAL,shares:0,equity:STARTING_CAPITAL,peak:STARTING_CAPITAL,reward:0,lastReward:0,vol:.016,maxDrawdown:.05,evalMinConfidence:.65,activeMinConfidence:.4,regime:'CALM',phase:'TRAIN',bars:[],trades:[],history:persistedHistory,pending:null,holdingBars:0,cooldown:0,tradeCount:0,turnover:0,stepTurnover:0,tradeStartEquity:null,tradeReward:0,ending:false,decision:{action:6,confidence:Array(7).fill(1/7),active:[]}};
const worker=new Worker('snn-worker.js?v=5');let timer,episodeTimer,brainNodes=[],workerInitialized=false;
...
let rand=seededRng(42);const normal=()=>normalFrom(rand);
const confidenceFor=()=>state.phase==='TRAIN'?(state.history.filter(x=>x.phase==='TRAIN').length<30?.4:state.history.filter(x=>x.phase==='TRAIN').length<100?.5:.6):state.evalMinConfidence;
...
function reset(){
  ...
  worker.postMessage(workerInitialized?{type:'reset',seed:marketSeed}:{type:'init',seed:381,stateSeed:marketSeed,control:CONTROL_MODE});workerInitialized=true;
...
function queueDecision(action,confidence,explored=false){
  if(state.ending||state.pending?.force||(!explored&&confidence<state.activeMinConfidence)||action===3||action===6)return;
  const exposure=state.equity?state.shares*state.price/state.equity:0,target=[.1,explored?.1:.25,explored?.1:.5,exposure,exposure/2,0,exposure][action];
  if(action<3&&(state.cooldown||state.tradeCount>=MAX_TRADES))return;
  if(action>=4&&state.shares>0&&state.holdingBars<MIN_HOLD)return;
  state.pending={action,target,confidence};
}
function executePending(open){
  ...
  const rawDelta=state.equity*target/open-state.shares,direction=Math.sign(rawDelta),fill=computeFill(open,direction,SPREAD_BPS,SLIPPAGE_BPS),delta=+(state.equity*target/fill-state.shares).toFixed(6);
  if(Math.abs(delta)<.000001)return;const notional=Math.abs(delta)*fill,commission=computeCommission(notional,COMMISSION_BPS);
  ...
  if(delta>0&&state.tradeStartEquity===null){state.tradeStartEquity=state.equity;state.holdingBars=0}else if(Math.abs(state.shares)<.000001){state.shares=0;state.cooldown=COOLDOWN;state.tradeReward=(state.cash-state.tradeStartEquity)/STARTING_CAPITAL*2;state.tradeStartEquity=null}
...
function marketStep(decide=true){
  ...
  const dd=(state.peak-state.equity)/state.peak,ddIncrease=Math.max(0,dd-previousDd),exposure=state.equity?state.shares*close/state.equity:0,opportunityCost=Math.max(0,close/open-1)*(1-exposure)*.05;
  state.lastReward=stepReward({equityBefore:previousEquity,equityAfter:state.equity,tradeReward:state.tradeReward,ddIncrease,stepTurnover:state.stepTurnover,opportunityCost,startingCapital:STARTING_CAPITAL});state.tradeReward=0;state.reward+=state.lastReward;
  if(state.shares&&state.tradeStartEquity&&(state.tradeStartEquity-state.equity)/STARTING_CAPITAL>=TRADE_RISK)state.pending={action:5,target:0,confidence:1,force:true};
  if(state.equity/STARTING_CAPITAL-1<=-EPISODE_LOSS){state.reward-=.5;finishEpisode('EPISODE LOSS '+pct(EPISODE_LOSS),-.5);render();return}
...
function liquidate(reason){
  if(!state.shares)return;const fill=computeFill(state.price,-1,SPREAD_BPS,SLIPPAGE_BPS),notional=state.shares*fill,commission=computeCommission(notional,COMMISSION_BPS);state.cash+=notional-commission;state.turnover+=notional/STARTING_CAPITAL;log('SELL '+state.shares.toFixed(4)+' · '+reason);state.shares=0;state.equity=state.cash;if(state.tradeStartEquity!==null){state.tradeReward=(state.cash-state.tradeStartEquity)/STARTING_CAPITAL*2;state.tradeStartEquity=null}
}
function finishEpisode(reason,terminalReward){
  ...
  state.history.push({version:2,mode:CONTROL_MODE?'control':'trained',episode:state.episode,phase:state.phase,return:strategyReturn,benchmark:benchmarkReturn,reward:state.reward+state.tradeReward,trades:state.tradeCount,turnover:state.turnover,reason});state.history=state.history.slice(-500);save(HISTORY_KEY,state.history);
...
worker.onmessage=e=>{
  if(e.data.type==='ready'){const checkpoint=CONTROL_MODE?null:load(CHECKPOINT_KEY,null);$('engine').textContent=e.data.engine;brainNodes=Array.from({length:96},(_,i)=>({x:rand(),y:rand(),p:i}));if(checkpoint)worker.postMessage({type:'restore',checkpoint});return}
  if(e.data.type==='checkpoint'){if(!CONTROL_MODE)save(CHECKPOINT_KEY,{weights:e.data.weights,learningUpdates:e.data.learningUpdates,totalAbsDelta:e.data.totalAbsDelta,decisions:e.data.decisions});return}
  if(e.data.type==='restored'){$('updates').textContent=e.data.learningUpdates.toLocaleString();log('Neural checkpoint restored');return}
  state.decision=e.data;$('action').textContent=ACTIONS[e.data.action]+(e.data.explored?' · EXPLORE':'');$('spikeRate').textContent=(e.data.total*62.5).toFixed(0)+' spikes/s';$('updates').textContent=e.data.learningUpdates.toLocaleString();$('weightDelta').textContent='ΔW '+e.data.meanDelta.toFixed(5)+' · ε '+(state.phase==='TRAIN'?(e.data.epsilon*100).toFixed(0)+'%':'OFF');const groups=[e.data.confidence.slice(0,3).reduce((a,b)=>a+b,0),e.data.confidence[3]+e.data.confidence[6],e.data.confidence[4]+e.data.confidence[5]];['buyBar','holdBar','sellBar'].forEach((id,i)=>$(id).style.height=(8+groups[i]*90)+'%');queueDecision(e.data.action,e.data.confidence[e.data.action],e.data.explored);
};
...
function renderPerformance(){
  const all=state.history,tests=all.filter(x=>x.phase==='TEST'),validations=all.filter(x=>x.phase==='VALIDATION'),h=tests.length>=5?tests:validations.length?validations:all,scope=tests.length>=5?'TEST':validations.length?'VAL':'ALL',wins=h.filter(x=>x.return>0),losses=h.filter(x=>x.return<0),avg=values=>values.length?values.reduce((a,b)=>a+b,0)/values.length:0,profit=wins.reduce((a,x)=>a+x.return,0),loss=Math.abs(losses.reduce((a,x)=>a+x.return,0));
  $('completed').textContent=all.length+' · '+scope;$('winRate').textContent=h.length?pct(wins.length/h.length):'—';$('avgReturn').textContent=h.length?pct(avg(h.map(x=>x.return))):'—';$('alpha').textContent=h.length?pct(avg(h.map(x=>x.return-x.benchmark))):'—';$('profitFactor').textContent=loss?(profit/loss).toFixed(2):profit?'∞':'—';$('turnover').textContent=state.turnover.toFixed(1)+'×';
}
...
$('confidenceLimit').oninput=e=>{state.evalMinConfidence=+e.target.value/100;if(state.phase!=='TRAIN')state.activeMinConfidence=state.evalMinConfidence;$('confidenceOut').textContent=Math.round(state.activeMinConfidence*100)+'%'};addEventListener('resize',drawBrain);reset();schedule();
